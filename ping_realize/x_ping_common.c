...
static int __g_oom_count;
static int __g_tokens;

static unsigned long __g_waittime;
...
{
	/** \brief because number 0 and 2 is used, so, ret = 1 , if return 1, sucess, else fail */
	int ret = 1;  
...
int __schedule_exit(int next)
{
	...
	if (nreceived) {
		__g_waittime = 2 * tmax;
		if (__g_waittime < 1000*interval)
			__g_waittime = 1000*interval;
	} else
		__g_waittime = lingertime*1000;

	if (next < 0 || next < __g_waittime/1000)
		next = __g_waittime/1000;

	it.it_interval.tv_sec = 0;
	it.it_interval.tv_usec = 0;
	it.it_value.tv_sec = __g_waittime/1000000;
	it.it_value.tv_usec = __g_waittime%1000000;
...
resend:
	...
	if (i > 0) {
		...
		if (nores_interval > 500)
			nores_interval = 500;
		__g_oom_count++;
		if (__g_oom_count*nores_interval < lingertime)
...
int main_loop(int icmp_sock, __u8 *packet, int packlen)
{
	...
	mark = 0;
	rtt_addend = 0;
	memset(&g_ping_data, 0, sizeof(ping_finish_data_t));
...
int finish(void)
{
	...
	ping_finish_data_t *p_data = &g_ping_data;
	char *comma = "";
	...
	strcpy(p_data->host_name, hostname);
	p_data->n_trans      = ntransmitted;
	p_data->received     =  nreceived;
	p_data->nrepeats     = nrepeats;
	p_data->nchecksum    = nchecksum;
	p_data->nerrors      = nerrors;
	p_data->package_lose = (int) ((((long long)(ntransmitted - nreceived)) * 100) / ntransmitted);
	p_data->sum_timeout  = (1000*tv.tv_sec+tv.tv_usec/1000);
	...
	if (nreceived && timing) {
		...
		p_data->i_min = ((long)tmin/1000);
		p_data->d_min = ((long)tmin%1000);

		p_data->i_max = ((long)tmax/1000);
		p_data->d_max = ((long)tmax%1000);	

		p_data->i_avg = ((long)tsum/1000);
		p_data->d_avg = ((long)tsum%1000);	

		p_data->i_mdev  = ((long)tmdev/1000);
		p_data->d_mdev  = ((long)tmdev%1000);
...
int ping_finish_data_get(ping_finish_data_t *p_dat)
{
   strcpy(p_dat->host_name, g_ping_data.host_name);
    p_dat->n_trans        = g_ping_data.n_trans;
    p_dat->received       = g_ping_data.received;
    p_dat->nrepeats       = g_ping_data.nrepeats;
    p_dat->nchecksum      = g_ping_data.nchecksum;
    p_dat->nerrors        = g_ping_data.nerrors;
    p_dat->package_lose   = g_ping_data.package_lose;
    p_dat->sum_timeout    = g_ping_data.sum_timeout;
    p_dat->i_min          = g_ping_data.i_min;
    p_dat->d_min          = g_ping_data.d_min;
    p_dat->i_avg          = g_ping_data.i_avg;
    p_dat->d_avg          = g_ping_data.d_avg;
    p_dat->i_max          = g_ping_data.i_max;
    p_dat->d_max          = g_ping_data.d_max;
    p_dat->i_mdev         = g_ping_data.i_mdev;
    p_dat->d_mdev         = g_ping_data.d_mdev;

	return 0;
}

void ping_finish_clr(void)
{
