...
typedef struct ping_finish_data {
	char   host_name[64];
	long    n_trans;
	long    received;
...
// int main_loop(int icmp_sock, __u8 *buf, int buflen) __attribute__((noreturn));
int main_loop(int icmp_sock, __u8 *buf, int buflen);
// extern void finish(void) __attribute__((noreturn));
int finish(void);
extern void status(void);
extern int common_options(int ch);
